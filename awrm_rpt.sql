...
col db_hosts heading "DB Hosts" format a15
col db_name heading "DB Name" format a8
col nbr_instances heading "#|inst" format 9990
...
col gib heading "Size|(GiB)" format 999,990
...
break on report on db_hosts skip page
compute sum of gib on db_hosts
...
from	awr_miner_info	i,
	awr_miner_disk	d
where	d.db_name = i.db_name
order by db_hosts, db_name, nbr_instances;
spool off
clear breaks computes
